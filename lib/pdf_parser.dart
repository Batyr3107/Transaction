...
class KaspiPdfParser {
  ...
  Map<String, String>? _extractTransferInfo(String line, List<String> lines, int index) {
    // Паттерн 1: "Перевод" + имя человека (например "Динара З.", "Раушан Н.", "Динара Жунусова")
    // Ищем "Перевод" и после него имя с фамилией (полной или сокращённой)
    if (line.contains('Перевод')) {
      // Паттерн имени: Имя + пробел + фамилия (полная или первая буква с точкой)
      final namePattern = RegExp(r'([А-ЯЁA-Z][а-яёa-z]+\s+[А-ЯЁA-Z](?:[а-яёa-z]+|\.)?)');
